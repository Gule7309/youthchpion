export const indicatorHelp = {
  S: { title: '實驗性結構暴露', meaning: '綜合青年集中程度與 AI 職務暴露，描述該職業面對 AI 變化的結構性暴露。', formula: 'S = 100 × √(A × B)', method: '目前後端只將 A 與 B 以同一尺度組合，H 與 D 獨立呈現；C 尚未完成可靠的產業到職業映射，因此不計入完整 Risk。這是團隊定義的實驗性指標，不是 AI 自動產生的公式。', interpretation: '數值越高，代表青年集中與 AI 職務暴露的組合訊號較高；不代表相同比例的人會失業或被取代。', limitation: '這不是完整 Risk。C 與校準尚未完成，系統必須讓完整風險分數保持空值。' },
...
export function IndicatorExplanation({ id }: { id: IndicatorId }) {
  ...
  return <div className="indicator-explanation">
    ...
    {id === 'S' && <p>完整 Risk 的正式公式與校準尚未發布。Risk 也不是失業機率；缺少 C 等必要資料時必須保持空值。</p>}
  </div>
}
