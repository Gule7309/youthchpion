export const indicatorHelp = {
  S: { title: '實驗性結構暴露', meaning: '綜合青年集中程度與 AI 職務暴露，描述該職業面對 AI 變化的結構性暴露。', formula: 'S = 100 × √(A × B)', method: '目前後端將 A 與 B 以同一尺度組合，H、D 與 C 獨立呈現。這是團隊定義的實驗性指標，不是 AI 自動產生的公式。', interpretation: '數值越高，代表青年集中與 AI 職務暴露的組合訊號較高；不代表相同比例的人會失業或被取代。', limitation: '目前 A 使用 20–24 歲資料，尚未涵蓋產品目標的完整 18–35 歲族群；跨來源分類與權重仍需持續核對。' },
  A: { title: '青年集中程度', meaning: '看這個職業的就業者中，有多少比例屬於目前資料契約定義的青年範圍。', formula: 'A = 該職業青年就業人數 ÷ 該職業全部就業人數', method: '分子與分母須使用相同職業分類、期間與地區。目前後端快照先提供 20–24 歲；團隊產品目標仍為 18–35 歲，待後端更新年齡範圍後同步。', interpretation: '比例越高，代表這個職業的變化可能涉及較多青年；A 本身不是 AI 暴露分數。', limitation: '年齡範圍與人口權重必須以同一次資料快照為準。分母是該職業的全部就業者，不是所有職業的青年總數。' },
  ...
  D: { title: 'AI 人才需求', meaning: '觀察該職業的招募中，有多少職缺要求 AI 技能，輔助討論技能需求與轉型方向。', formula: 'AI 技能職缺占比 = 該職業 AI 技能職缺數 ÷ 該職業有效職缺總數\n以百分比呈現時，再乘以 100%', method: '先確認擷取範圍、去重與有效職缺，再按核定的 AI 技能判定規則計數。相對百分位還需要固定比較母體；成長率需要同口徑的歷史資料。', interpretation: '需求占比高不代表青年一定容易找到工作，還要看經驗與技能要求。D 獨立呈現，不直接納入結構暴露。', limitation: '職缺覆蓋、AI 判定、排名母體及可比歷史尚未完整，因此占比、排名與成長率目前都不提供正式數值。' },
} as const
...
export function IndicatorExplanation({ id }: { id: IndicatorId }) {
  ...
  return <div className="indicator-explanation">
    ...
    {id === 'S' && <p>結構暴露用於比較不同職業的相對訊號，不是失業率，也不是個人被 AI 取代的機率。</p>}
  </div>
}
