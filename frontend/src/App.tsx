...
export default function App() {
  ...
  return <div className="folder-app policy-dashboard" data-modal-open={modalOpen}>
    ...
    <dialog ref={dialog} className="review-dialog" onClose={() => setModalOpen(false)} aria-labelledby="detail-title" onCancel={e => { e.preventDefault(); closeDetail() }} onClick={e => {
        ...
        <h2 id="detail-title">{occupation.name} · {evidenceId ? '證據詳情' : detail}</h2>
        ...
        {!evidenceId && detail === '計算方式' && <><p>年度變化 =（2025 − 2024）÷ 2024 = {percentage(occupation.previous, occupation.current)}。零基期或缺值不計算。</p><p>團隊候選模型：S = (A × B × C)^(1/3)；Risk = 100 × (S + H) / 2。D 獨立呈現。H = clip(−年度變化 / 0.20, 0, 1)，尚待核定。</p><p>AI 不可任意改寫公式。模型不是失業機率；缺少必要指標不補零、不計分。</p></>}
        {!evidenceId && detail === '清洗紀錄' && <><ol><li>由官方 JSON 選取 113、114 年及對應職業。</li><li>將求才人次轉成數值；確認兩年職業名稱與單位相同。</li><li>計算年度變化率，顯示至小數點後兩位。</li></ol><p>目前為已核對本地快照，不是即時清洗日誌。新版管線的步驟、筆數、SHA-256 與 crosswalk 稽核仍待 API 提供。</p></>}
        {!evidenceId && detail === '估算方式' && <>{pendingIndicators.map(m => <section key={m.id}><h3>{m.id} · {m.name}</h3><p>{m.reason}</p><a href={m.source} target="_blank" rel="noreferrer">來源 ↗</a></section>)}<p>C 需以職業在各產業的就業權重轉換；產業不是主比較維度。估算／proxy、覆蓋率、信心須由核定資料提供，目前均未定。</p></>}
      </dialog>
  </div>
}
