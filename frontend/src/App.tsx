...
function indicator(signal: OccupationSignal, id: string) {
  ...
  if (id === 'D') return { value: percent(signal.ai_entry_opportunity_rate), ratio: signal.ai_entry_opportunity_rate, note: `${number.format(signal.ai_entry_jobs)} 個 AI 相關機會；樣本品質另揭露` }
...
export default function App() {
  ...
  async function refresh() {
    ...
    setBusy('QUEUED')
    setError('')
    setNotice('正在重新連線五個來源並建立新的可稽核快照。')
    try {
  ...
  const renderPage = (index: number) => {
    if (index === 0) return <aside className="panel policy-indicators" aria-label="目前職業核心指標">
      <div className="section-kicker"><Icon name="formula" />核心指標</div>
      <h2>{selected?.name}</h2>
      ...
      {([['A', '職業內青年占比'], ['B', 'AI 職業暴露'], ['H', '官方招募弱化'], ['D', 'AI 初階機會'], ['C', '台灣產業導入']] as const).map(([id, label]) => {
        ...
        return <div className="policy-metric" key={id}>
          ...
          <div><span>{label}</span><small>{metric.note}</small></div>
    ...
    if (index === 1) return <div className="risk-layout">
      ...
      <section className="panel policy-comparison" aria-labelledby="comparison-title">
        <div className="panel__header"><div><div className="section-kicker"><Icon name="users" />職業比較</div><h2 id="comparison-title">哪些職業值得優先關注？</h2></div><span className="status">{visibleSignals.length} 個職業</span></div>
        ...
        <div role="group" aria-label="選擇職業">{visibleSignals.map((item) => <button className="policy-occupation" aria-pressed={selected?.code === item.code} key={item.code} onClick={() => selectOccupation(item.code)}>
          ...
          <span><b>{score(item.structural_exposure_score)}</b><small>{item.score_status ?? item.priority}</small></span>
    ...
    if (index === 2) return <aside className="panel policy-diagnosis">
      ...
      <div className="diagnostic-summary"><span className="diagnostic-icon"><Icon name="info" /></span><span className="policy-diagnostic-label">{selected?.priority === 'high' ? '相對高結構暴露' : selected?.priority === 'medium' ? '相對中段' : '持續觀察'}</span><p>實驗性結構暴露 {score(selected?.structural_exposure_score)}；20–24 歲就業 {number.format(selected?.youth_employed ?? 0)} 人，占該職業 {percent(selected?.youth_employment_share)}；官方求才年變化 {percent(selected?.recruitment_yoy_change)}。</p></div>
      <h3>判讀重點</h3><ul><li>A×B 只形成結構暴露排序；不是失業、取代率或完整 Risk。</li><li>H 來自獨立官方求才序列；D 是另一份即時職缺機會訊號，不互相反轉。</li><li>C 尚無可靠職業量化值，因此完整 Risk 保持空值。</li><li>公眾感受獨立呈現，不冒充客觀風險或因果。</li></ul>
    ...
    if (index === 3) return <section className="panel" id="research" aria-labelledby="research-title">
      ...
      <div className="policy-evidence-list">{evidence.slice(0, 6).map((item) => <label className="policy-evidence" key={item.evidence_id}><input type="checkbox" checked={selectedEvidence.includes(item.evidence_id)} onChange={() => { setSelectedEvidence((current) => current.includes(item.evidence_id) ? current.filter((id) => id !== item.evidence_id) : [...current, item.evidence_id].slice(-3)); setVerification(null); setPolicy(null) }} /><span><b>{item.title}</b><em>{item.authority_tier} 級候選 · {item.institution} · {item.discovery_source ?? '既有資料'} · {item.freshness}</em><p>{item.finding ?? '目前只有索引 metadata；Agent 不會把它直接當成主張證據。'}</p><a href={item.url} target="_blank" rel="noreferrer">查看候選來源 <Icon name="external" /></a></span></label>)}</div>
    ...
    return <section className="panel policy-report" id="policy" aria-labelledby="policy-title">
      ...
      {!policy && <><h3>先通過權威證據閘門，再收斂政策</h3><p>政策 API 只接受本次分析版本中、已被 Agent 取回原文並通過 publication gate 的證據 ID。</p><div className="policy-report-outline"><span>政策輸出會比較</span><ol><li>介入機制與目標族群</li><li>執行步驟與 KPI</li><li>證據、風險與限制</li></ol></div><button className="button" onClick={runPolicy} disabled={Boolean(busy) || !verification?.approved_evidence_ids.length}><Icon name="report" />{busy === 'POLICY' ? '推論中…' : '產生三個政策選項'}</button><p className="policy-muted">不匯出報告；現場直接比較機制、執行、KPI、風險與限制。</p></>}
      {policy && <div className="policy-options">{policy.options.map((option, optionIndex) => <article key={option.title}><span>0{optionIndex + 1}</span><h3>{option.title}</h3><p><b>{option.target_group}</b>｜{option.mechanism}</p><ol>{option.implementation.map((step) => <li key={step}>{step}</li>)}</ol><details data-memory={`policy-${optionIndex}`}><summary>證據、風險與限制</summary><p>證據：{option.evidence_ids.join('、')}</p>{[...option.risks, ...option.limitations].map((item) => <p key={item}>{item}</p>)}</details></article>)}</div>}
    </section>
  }
  ...
  return <div className="folder-app policy-dashboard" data-modal-open={modalOpen}>
    ...
    <FolderWorkspace ref={workspace} contextKey={`${dashboard.analysis_run_id}:${selected?.code ?? ''}`} blocked={modalOpen} onExternalNavigate={() => closeDetail(false)} renderPage={renderPage} />
    ...
    <dialog ref={dialog} className="review-dialog" aria-labelledby="detail-title" onClose={() => setModalOpen(false)} onCancel={(event) => { event.preventDefault(); closeDetail() }} onClick={(event) => {
      ...
      <div className="dialog-heading"><span className="section-kicker"><Icon name={DETAIL_ICONS[detail]} />分析詳情</span><button className="dialog-close" autoFocus onClick={() => closeDetail()} aria-label="關閉詳情"><Icon name="close" /></button></div>
      <h2 id="detail-title">{selected?.name} · {detail}</h2>
      ...
      {detail === '計算方式' && <><p><b>實驗性結構暴露 = 100 × √(A × B)</b></p><p>A：該職業內 20–24 歲就業人數／該職業全部就業人數；P：該職業占全部 20–24 歲就業的比率；B：ILO 生成式 AI 任務暴露 proxy；H：官方求才年減的獨立弱化訊號；D：台灣就業通 AI 初階職缺占比。H 與 D 均未塞入目前 A×B 排序。</p><p>目前選取：A={percent(selected?.youth_employment_share)}、P={percent(selected?.occupation_share_of_youth)}、B={score(selected?.exposure_score)}、H={percent(selected?.recruitment_weakening)}、D={percent(selected?.ai_entry_opportunity_rate)}；結構暴露 {score(selected?.structural_exposure_score)}，完整 Risk={score(selected?.complete_risk_score)}。</p><p>{String(metrics.metric_warning ?? '')}</p><code>{String(metrics.score_version ?? '')}</code></>}
      ...
      {detail === '估算限制' && <><p>A×B 是跨來源的實驗性結構暴露排序，不是 AI 造成失業的因果估計，也不是個人被取代機率。</p><p>C（台灣產業實際 AI 導入）目前只有產業研究脈絡，缺少可比的「產業→職業」權重，所以完整 Risk 明確保持空值。</p><p>H 只代表公立就業服務求才弱化，不能歸因於 AI；D 的抽樣、分類與 crosswalk 品質仍需逐項顯示。民意調查只反映主觀感受。</p></>}
    </dialog>
  </div>
}
