...
describe('五頁檔案夾工作區', () => {
  ...
  it('同一外夾由側邊變主頁時保留 DOM，裝飾不建立第二組可操作內容', () => {
    ...
    go('風險'); expect(document.querySelector('[data-folder="risk"]')).toBe(shell)
    go('報告'); go('指標'); expect(document.querySelector('[data-folder="risk"]')).toBe(shell)
  ...
  it('即使系統 reduce，網站仍預設開啟，使用者可關閉且重新進入仍保留', () => {
    ...
    go('報告');expect(document.querySelector('.is-transitioning')).toBeNull()
    cleanup();render(<App/>)
    expect(screen.getByRole('switch',{name:'動畫效果'})).toHaveAttribute('aria-checked','false')
    fireEvent.click(screen.getByRole('switch',{name:'動畫效果'}))
  ...
  it('初始只有指標頁可讀，五個分頁各有對應面板', () => {
    render(<App/>)
    expect(screen.getAllByRole('tab').map(t=>t.textContent)).toEqual(['指標','風險','診斷','論證','報告'])
  ...
  it('五頁內容分工與真實資料限制保留，換頁不觸發 fetch', () => {
    ...
    go('論證'); expect(within(panel()).getByText('模型方法來源')).toBeInTheDocument()
    go('報告'); expect(within(panel()).getByRole('button',{name:'產生完整政策報告'})).toBeDisabled()
    expect(fetch).not.toHaveBeenCalled()
  })
  it('頂部與側邊直接跨頁，重复點選不增加歷史', () => {
    render(<App/>); const push=vi.spyOn(window.history,'pushState')
    go('報告'); expect(panel()).toHaveAttribute('id','panel-report')
    expect(push).toHaveBeenCalledTimes(1)
    go('報告'); expect(push).toHaveBeenCalledTimes(1)
  ...
  it.each([['指標',[1,2]],['風險',[0,2,3]],['診斷',[0,1,3,4]],['論證',[1,2,4]],['報告',[2,3]]] as const)('側邊排列：%s', (label, expected) => {
  ...
  it('切職業不換頁，其他頁與來源詳情使用同一職業', () => {
    ...
    expect(within(panel()).getByRole('heading',{name:'專業人員的研究依據'})).toBeInTheDocument()
    go('風險'); expect(within(panel()).getByText('118,244 人次')).toBeInTheDocument()
    expect(within(panel()).getByRole('button',{name:/職業大類 2/})).toHaveAttribute('aria-pressed','true')
    go('報告'); expect(within(panel()).getByRole('heading',{name:'專業人員的政策方向'})).toBeInTheDocument()
  })
  it('鍵盤方向鍵只移動焦點，不循環，側邊與 Tab 只導向有效內容', () => {
    render(<App/>)
    act(()=>tab('指標').focus())
    fireEvent.keyDown(tab('指標'),{key:'ArrowLeft'}); expect(tab('指標')).toHaveFocus()
    fireEvent.keyDown(tab('指標'),{key:'End'}); expect(tab('報告')).toHaveFocus()
    expect(tab('指標')).toHaveAttribute('aria-selected','true')
    fireEvent.keyDown(tab('報告'),{key:'ArrowRight'}); expect(tab('報告')).toHaveFocus()
    // jsdom doesn't synthesize a button click from Enter; browser coverage handles that.
    fireEvent.click(tab('報告')); expect(tab('報告')).toHaveAttribute('aria-selected','true')
  ...
  it('reduce 初始與動態改變皆直接定格且可繼續切頁', () => {
    ...
    motion.matches=true; render(<App/>); go('報告')
    expect(document.querySelector('.is-leaving')).toBeNull()
    fireEvent.click(screen.getByRole('switch',{name:'動畫效果'}))
  ...
  it('modal 隔離背景，關閉回焦點，外部導航可安全離開', () => {
    ...
    expect(tab('報告')).toBeDisabled(); go('報告');expect(window.location.hash).toBe('#evidence')
    ...
    expect(screen.queryByRole('dialog')).toBeNull();expect(tab('報告')).toHaveFocus()
  })
  it.each(['failed','incompatible'])('刷新 %s 保留快照與報告限制', async mode => {
    ...
    expect(await screen.findByText(mode==='failed'?/重新抓取未完成/:/回傳仍為舊/)).toBeInTheDocument()
    go('風險');expect(within(panel()).getByText('109,342 人次')).toBeInTheDocument()
