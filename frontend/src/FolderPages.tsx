...
export type PageProps = { occupation: Occupation; code: string; setCode: (code: string) => void; openDetail: (kind: Detail, button: HTMLButtonElement) => void }

export function IndicatorsPage({ occupation, openDetail }: PageProps) { return <aside className="panel policy-indicators" aria-label="目前職業核心指標">
          <div className="section-kicker"><Icon name="chart" /><span>核心指標</span></div>
          ...
          {metrics.map(([id, label]) => <div className="policy-metric" key={id}>
            ...
            <div><span>{label}</span><small>{id === 'H' ? '歷史已取得 · 方法待核定' : '待核對'}</small></div>
...
export function RiskPage({ occupation, code, setCode }: PageProps) { return <div className="risk-layout"><aside className="risk-overview"><span className="section-kicker">目前職業的風險</span><h2>{occupation.name}</h2><div className="policy-risk">
          ...
          </div><p className="policy-muted">此模型為早期預警指標，不是失業機率。</p></aside><section className="panel policy-comparison" id="comparison" aria-labelledby="comparison-title">
          <div className="panel__header"><div><div className="section-kicker"><Icon name="users" /><span>職業比較</span></div><h2 id="comparison-title">哪些職業值得優先關注？</h2></div><span className="status">3 個職業</span></div>
...
export function DiagnosisPage({ occupation, openDetail }: PageProps) { return <aside className="panel policy-diagnosis">
...
export function EvidencePage({ occupation, openDetail }: PageProps) { return <div><section className="panel" id="research" aria-labelledby="research-title">
          ...
          <small>以上為方法／背景來源。職業專屬的支持、反對證據與政策研究原文定位仍待補齊，不冒充專家背書。</small>
...
export function ReportPage({ occupation }: PageProps) { return <section className="panel policy-report" id="policy" aria-labelledby="policy-title">
          ...
          <h3>先確認問題，再選擇介入方式</h3><p>現有招募趨勢不足以支持特定政策。需確認青年／初階職缺是否有相同變化，以及景氣、產業結構等其他解釋。</p>
          <div className="policy-report-outline"><span>完整政策報告將包含</span><ol><li>職業風險與指標診斷</li><li>國際研究與適用限制</li><li>政策選項、執行措施與 KPI</li></ol></div>
          <button className="button" disabled aria-describedby="report-reason"><Icon name="report" />產生完整政策報告</button>
          <p id="report-reason" className="policy-muted">新版模型、政策證據及報告 API 尚未就緒，暫不產生報告。</p>
        </section> }
