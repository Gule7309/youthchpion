...
### 五頁資料消費對應（不是既有 API schema）

| 頁面 | 消費資料 |
|---|---|
| 指標 | 同職業 S/A/B/C/H/D、各項狀態與方法 |
| 風險 | Risk 或不可用原因、可比較職業及真實招募趨勢 |
| 診斷 | 核定型態、理由與資料品質限制 |
| 論證 | 同職業研究引用與來源／清洗／估算詳情 |
| 報告 | 同職業政策選項與報告服務；未就緒維持停用 |

換頁不得觸發 ingestion 或生成。五頁共用 snapshotId 與 occupationCode；未来完整發布版原子替換，舊契約不得改標籤冒充新模型。本轮未新增或更改端點。
